...
def run_test(request):
    ...
    if request.is_ajax():
        ...
        run_test_by_type(id, base_url, env_token, env_merchantId, testcase_dir_path, type)
        ...
        runner.summary = timestamp_to_datetime(runner.summary, base_id, type=False)

        return render_to_response('report_template.html', runner.summary)
        # return render(relpace_report, data=runner.summary)
...
def run_batch_test(request):
    ...
    if request.is_ajax():
        ...
        env_merchantId = env_values[0].get('env_merchantId')
        report_name = kwargs.get('report_name', None)
        run_by_batch(test_list, base_url, env_token, env_merchantId, testcase_dir_path, type=type)
        # main_hrun.delay(testcase_dir_path, report_name)
        main_hrun(testcase_dir_path, report_name, base_id)
        return HttpResponse('用例执行中，请稍后查看报告即可,默认时间戳命名报告')
    else:
        ...
        env_merchantId = env_values[0].get('env_merchantId')
        if type:
            run_by_batch(test_list, base_url, env_token, env_merchantId, testcase_dir_path, type=type, mode=True)
        else:
