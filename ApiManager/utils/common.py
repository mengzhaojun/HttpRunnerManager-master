...
from ApiManager.models import ModuleInfo, TestCaseInfo, TestReports, TestSuite, EnvInfo
...
def case_info_logic(type=True, **kwargs):
    ...
    if 'request' not in test.keys():
        ...
        if test.get('name').get('module') == '':
            return '请添加模块'
        if test.get('name').get('run_type') == '0':
            return '请选择运行方式'
...
def timestamp_to_datetime(summary, env_id, type=True):
    if not type:
    ...
    # 提取token值
    # print('token: ', summary['details'][0]['records'][0]['meta_data']['request']['url'])
    message = summary['details'][0]['records'][0]['meta_data']['request']['url']
    if 'login/login' in message:
        # print('---执行login----')
        res = summary['details'][0]['records'][0]['meta_data']['response']['content']
        result = json.loads(res)
        token = result['result']['token']
        merchantId = result['result']['userModel']['merchantId']
        EnvInfo.objects.filter(pk=env_id).update(env_token=token, env_merchantId=merchantId)
    return summary
